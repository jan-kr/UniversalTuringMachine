...
public class UniversalTuringMachine {
    ...
    public void printTransitionTable() {
        ...
        for (int i = 0; i < arrayList.length; i++) {
            for (Transition transition : arrayList[i]) {
                System.out.printf("| %-12s | %-12s | %-12s | %-12c | %-12s |%n",
                        transition.getState(),
                        transition.getTapeValue(),
                        transition.getTapeValueNew(),
    ...
    private int getHighestState() {
        int highestTransition = 0;
        for (Transition transition : transitionSet) {
            int temp = Integer.parseInt(transition.getNewState().split("q")[1]);
            if (temp > highestTransition) {
                highestTransition = temp;
            }
        }
